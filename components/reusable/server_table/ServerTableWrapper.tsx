...
const ServerPagination = async (props: ServerPaginationProps) => {
  ...
  return (
    <div className='flex items-center justify-between'>
      ...
      <ServerPageTableControls totalCount={data.count}/>
    </div>
  )
}
...
const ServerTableWrapper = <T, >(props: ServerTableWrapperProps<T>) => {
  ...
  return (
    <>
      <Suspense fallback={<TableWrapperCore headers={props.headers} row={props.row} data={[]} isLoading/>}>
        <ServerTableData  
          {...props}
        />
      </Suspense>
      <div className='pt-4'/>
      <Suspense>
        <ServerPagination endpoint={props.endpoint} params={props.params} serverTag={props.serverTag}/>
      </Suspense>      
    </>
  )
}
