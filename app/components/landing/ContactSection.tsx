...
const ContactSection = ({ allSlots = [], title, description, padding = true }: ContactSectionProps) => {
  ...
  return (
    <section
      id="contacto"
      className="w-full flex flex-col justify-center items-center gap-2.5 min-h-162.5 px-25 pb-25">
      <div
        className={`w-full max-w-325 min-h-162.5 flex flex-col items-start gradient-deg1 gap-15 rounded-3xl bg-gray-200 lg:flex-row ${padding ? 'p-10' : '0'}`}>
        <div className="flex flex-col items-start justify-between flex-1 gap-10 min-h-142.5 ">
          <div className="flex flex-col items-start gap-10 w-full max-w-140">
            <div className="flex flex-col items-start gap-2.5">
              <h2 className="text-5xl font-semibold max-w-137.5 leading-12">
                {title}
              </h2>
            ...
            <div className="flex flex-col items-start gap-2.5">
              {
                contact === false ? (
                  <button
                    className="text-lg text-black cursor-pointer underline text-center"
                    onClick={() => {setContact(true), setShedule(false)}}
                  >
                  ...
                  <button
                    className="text-lg text-black cursor-pointer underline text-center"
                    onClick={() => {setContact(false), setShedule(true)}}
                  >
